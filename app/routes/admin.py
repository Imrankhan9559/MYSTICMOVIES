...
async def _render_main_settings(request: Request, user: User, speed_result: dict | None = None):
    ...
    mail_test_status = (request.query_params.get("mail_test") or "").strip().lower()
    mail_test_to = urllib.parse.unquote_plus((request.query_params.get("mail_test_to") or "").strip())
    mail_test_detail = urllib.parse.unquote_plus((request.query_params.get("mail_test_detail") or "").strip())
    return templates.TemplateResponse("main_settings.html", {
        ...
        **base_ctx,
        ...
        "mail_test_detail": mail_test_detail,
    })
...
@router.post("/admin/site/test-email")
async def test_site_email(
    request: Request,
    test_email: str = Form(""),
):
    user = await get_current_user(request)
    if not _is_admin(user):
        raise HTTPException(403)
    target = (test_email or "").strip().lower()
    if not re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", target, re.I):
        return RedirectResponse("/main-settings?mail_test=invalid", status_code=303)

    site = await _site_settings()
    subject = f"{(site.site_name or 'MysticMovies').strip()}: SMTP Test"
    html_body = build_email_html(
        subject="SMTP Test Email",
        preheader="This is a test email from MysticMovies.",
        greeting="Hello,",
        body_lines=[
            "SMTP settings are active and this test email was delivered successfully.",
            "If you received this, request and notification emails should work.",
        ],
        cta_text="Open Main Settings",
        cta_url="/main-settings",
        footer_note="Test email generated by admin panel.",
        brand_name=(site.site_name or "MysticMovies").strip(),
        accent=(site.accent_color or "#facc15").strip(),
    )
    text_body = "SMTP test email from MysticMovies."
    ...
    detail_q = urllib.parse.quote_plus(str(detail or "")[:240])
    to_q = urllib.parse.quote_plus(target)
    return RedirectResponse(
        f"/main-settings?mail_test={test_status}&mail_test_to={to_q}&mail_test_detail={detail_q}",
        status_code=303,
    )
